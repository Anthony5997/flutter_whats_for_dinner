  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocProvider<SessionBloc>(
        create: (context) =>
            SessionBloc()..add(SessionPageSelectedEvent(widget._index)),
            ...
            return Navigator(
              pages: [
                ...
                if (state is SessionLoadingState)
                  MaterialPage(child: LoadingScreenSession()),
                if (state is SessionFavoriteState)
                  MaterialPage(child: LoadingScreenSession()),
                if (state is SessionProfileState)
                  MaterialPage(child: LoadingScreenSession()),
                if (state is SessionPageSelectedState)
                  MaterialPage(child: LoadingScreen()),
                if (state is SessionIngredientCategoryState)
                  MaterialPage(child: IngredientCategorieView()),
                if (state is SessionRecipeListState)
                  MaterialPage(child: RecipeListView()),
